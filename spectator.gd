...
func _on_main_showing_loading_screen() -> void:
	...
	if vp:
		...
		$GXDKSpectatorCamera.show_camera_in_hmd = false
...
func _on_main_showing_loaded_scene() -> void:
	...
	if vp:
		if OS.has_feature("minimum_spectator"):
			...
			$GXDKSpectatorCamera.show_camera_in_hmd = true


func _on_camera_selection_dropdown_item_selected(index):
	match index:
		0:
			$GXDKSpectatorCamera.make_current()
		1:
			$GXDKStabilizedCamera.make_current()
